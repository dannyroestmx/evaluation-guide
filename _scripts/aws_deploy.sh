...
# for Production, want to set a redirect on /index file to point to https://www.mendix.com/evaluation-guide
# changed to /evaluation-guide to try to resolve the 502 bad gateway issues with WordPress 
if ([ "${TARGETAWSBUCKET}" == "evaluation-guide.mendix.com" ])
then
  aws s3 cp index s3://$TARGETAWSBUCKET --content-type text/html --website-redirect /evaluation-guide
fi
